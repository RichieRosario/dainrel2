const variables = require('./inscribir_variables');
...
function escribirOutput(codigo, cedulas, nombres) {



    var fs = require('fs');
    var dir = __dirname+'/output.csv';
    checkIfUnusedOutput(dir);
    var result = 'Codigo,Nombre,Cedula';
    for (var i = 0; i < nombres.length; i++) {
        result += '\r' + codigo + ',' + nombres[i] + ',' + cedulas[i]
    }
    ...
    fs.writeFile('output.csv', result, 'utf8', function(err) {
   	...
   	fs.rename('output1.csv', 'output.csv', (err) => {
...
const start = async function(username, password, password2, codigo, courseUrl, minGrade, esDiplomado, datosCSV,rl) {
    ...
    if (loggedin) {
        ...
        if (!gradesUrl) {
            ...
            for (var i = 0; i < nombresFinal.length; i++) {
                console.log("NOMBRE: " + nombresFinal[i] + ' |' + " CEDULA: " + cedulas[i])
            }

            await escribirOutput(codigo, cedulas,nombresFinal);
...
let nombreCSV = variables.nombreCSV();
...
fs.createReadStream(nombreCSV + '.csv', {
    ...
    .on('end', function() {
        ...
        for (var i = 1; i < dataArr.length; i++) {
            ...
            datosCSV.push({
                nombre: dataArr[i][0] + " " + dataArr[i][1],
                cedula: dataArr[i][2]
            });
        }
